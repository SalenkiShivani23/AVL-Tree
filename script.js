...
function findNode(node, value) { if (!node) return null; if (node.value===value) return node; return value<node.value ? findNode(node.left,value) : findNode(node.right,value); }
...
function searchValue(){
  ...
  searchPath=[];
  if(searchNodePath(root,val)) { searchedValue=val; alert("Value found in AVL Tree"); }
  else { searchedValue=null; alert("Value not found in AVL Tree"); }
...
function assignPositions(node,depth,pos){
  if(!node) return;
...
function drawCircle(x,y,value){
  ...
  if(value===searchedValue){ circle.setAttribute("stroke","#f43f5e"); circle.setAttribute("stroke-width","4"); }
  else if(searchPath.some(n=>n.value===value)){ circle.setAttribute("stroke","#fcd34d"); circle.setAttribute("stroke-width","4"); }
  else{ circle.setAttribute("stroke","#0f172a"); circle.setAttribute("stroke-width","2"); }
